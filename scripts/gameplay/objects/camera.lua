...
local round = function(number, idp)
...
function M.new(instance)
    ...
    camera.offsetH = 0
    camera.offsetV = 0
    camera.smoothingSpd = 4

    camera.debugMargin = {}

    camera.margin_v = true
    camera.margin_h = true
    ...
    function camera:moveFollowFocusObject()
        ...
        self.x = round(lerp(self.x, self.x + deltaX, smoothing))
        self.y = round(lerp(self.y, self.y + deltaY, smoothing))
    end

    local startTime = 0
    local function update(evt)
        if not camera then
            return false
        end

        local dt = evt.time - startTime
        startTime = evt.time
        ...
        world.x = world.x + objx + camera.offsetH * dt
        world.y = world.y + objy + camera.offsetV * dt
