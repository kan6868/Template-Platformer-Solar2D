...
local xml = require("scripts.libs.xml").newParser()
local json = require "json"

local M = {}
local defaultExtensions = "scripts.objects."
...
-- return all display objects with type
  ...
  function map:centerObject(obj, tween)
    ...
    local objx, objy = obj:localToContent(.5, .5)
  ...
  local function clamp(high, low, value)
    if value >= low and value <= high then
      return high
    end
  end
  function map:centerPlayer(obj, tween)
    -- moves the world, so the specified object is on screen
    if type(obj) == "string" then 
      obj = self:findObject(obj) 
    end
    ...
    -- obj.prevX, obj.prevY = obj.curX, obj.curY
    -- easiest way to scroll a map based on a character
    -- find the difference between the hero and the display center
    -- and move the world to compensate
    ...
    objx, objy = centerX - objx , centerY - objy
    -- objx, objy = math.ce(objx), math.floor(objy)
    -- objx.curX, objy.curY = objx, objy
    -- local x, y = clamp(obj.prevX, obj.curX, objx), clamp(obj.prevY, obj.curY, objy)
    if tween then
      self.x, self.y = self.x + objx/4, self.y + objy/4
    else
      self.x, self.y = self.x + objx, self.y + objy
