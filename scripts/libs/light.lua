...
function M.new(ambient, width, height, ox, oy)
  local width = width or display.actualContentWidth
  local height = height or display.actualContentHeight
  local ox = ox or display.screenOriginX
  local oy = oy or display.screenOriginY
  local lights = {}
  local dim = 0

  local instance = display.newSnapshot(width, height)
  instance.x, instance.y = ox, oy
  ...
  function instance:addLight(object, size)
    ...
    lights[id] = display.newImageRect(instance.group, "res/effects/light.png", size or 512, size or 512)
    lights[id].x, lights[id].y = object:localToContent(0,0)
    lights[id]:translate(-ox-width/2, -oy-height/2)    
    ...
    for k, v in pairs(lights) do
      if v and v.active then
        ...
        elseif v.objectToFollow.localToContent then
          ...
          v:translate(-ox-width/2, -oy-height/2)
